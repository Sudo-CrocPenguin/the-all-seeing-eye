...
class DetectMissedHeartbeatsUseCase:
    ...
    def execute(self, command: DetectMissedHeartbeatsCommand) -> list[AgentLifecycleEvent]:
        ...
        for device in self._device_repository.list_all():
            if not self._should_mark_missed(device, stale_before):
                continue
            if self._latest_event_explains_absence(device.device_id):
                continue

            event = self._build_missed_event(device, detected_at)
    ...
    def _latest_event_explains_absence(self, device_id: str) -> bool:
        latest_events = self._lifecycle_event_repository.search(
            AgentLifecycleEventFilters(device_id=device_id, limit=1),
        )
    ...
    @staticmethod
    def _build_missed_event(device: Device, detected_at: datetime) -> AgentLifecycleEvent:
